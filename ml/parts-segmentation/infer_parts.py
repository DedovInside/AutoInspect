#!/usr/bin/env python3
"""Inference script for AutoInspect Car Parts Segmentation.

Example:
    python infer_parts.py \
        --source "path/to/car.jpg" \
        --model "parts_segmentation.pt" \
        --imgsz 768 \
        --conf 0.25 \
        --iou 0.70 \
        --device auto \
        --save \
        --json "outputs/parts_predictions.json"
"""
...
DEFAULT_REPO_ID = "mitbersh/car-parts-segmentation"
DEFAULT_MODEL_FILENAME = "parts_segmentation.pt"
DEFAULT_PROJECT = "runs/parts_infer"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run AutoInspect car parts segmentation inference."
    )
    parser.add_argument(
        "--source",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "parts_segmentation.pt from Hugging Face."
        ),
    )
    parser.add_argument(
        "--repo-id",
...
def result_to_record(result: Any) -> dict[str, Any]:
    ...
    for idx, class_id in enumerate(class_ids):
        class_name = names.get(class_id, CLASS_NAMES[class_id] if class_id < len(CLASS_NAMES) else str(class_id))
