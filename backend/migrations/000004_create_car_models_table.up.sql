...
-- Уникальность активной версии модели
...
-- Быстрый поиск подходящей модели
CREATE INDEX idx_car_models_lookup
    ON car_models(make, model, generation, year_from)
    WHERE is_active = true;

-- Фоллбэк на универсальную модель
CREATE INDEX idx_car_models_universal
    ON car_models(is_universal)
